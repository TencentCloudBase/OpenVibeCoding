...
import { FileSystemLocalWorkspaceStore } from '../local-workspace-sync/index.js'
...
describe('LocalRuntimeSandbox', () => {
  ...
  it('creates a local workspace sync engine when a store is configured', async () => {
    const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), 'oak-local-test-'))
    const storeRoot = await mkdtemp(path.join(os.tmpdir(), 'oak-local-store-'))
    cleanup.push(workspaceRoot, storeRoot)

    const runtime = new LocalRuntimeSandbox({
      workspaceRoot,
      workspaceSyncStore: new FileSystemLocalWorkspaceStore({ root: storeRoot }),
    })
    const engine = runtime.createWorkspaceSyncEngine({
      envId: 'env-test',
      conversationId: 'conv-test',
      userId: 'user-test',
    })
